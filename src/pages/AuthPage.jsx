...
export function AuthPage({ mode = 'login' }) {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (mode === 'signup') {
        await signup(email, password, role, name, phone, gender, aadhar, address, idNumber);
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative">
      ...
      <Card className="w-full max-w-md animate-fade-in-up">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl">{mode === 'login' ? t('signIn', 'Sign In') : t('signUp', 'Sign Up')}</CardTitle>
          <CardDescription>
        ...
        <form onSubmit={handleAuth}>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('role', 'Role')}</label>
                ...
                {role === 'user' && (
                  <>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input placeholder="1234 5678 9012" required value={aadhar} onChange={e => setAadhar(e.target.value)} />
                    </div>
          ...
          <CardFooter className="flex flex-col gap-4">
            <Button type="submit" variant="gradient" className="w-full" disabled={loading}>
              {loading ? t('processing', 'Processing...') : (mode === 'login' ? t('signIn', 'Sign In') : t('signUp', 'Sign Up'))}
            </Button>
