...
export function ModeratorDashboard() {
  ...
  const normalizePriority = (priority) => priority === 'Medium' ? 'Moderate' : priority;
  const filteredReports = reports
    .filter(r => tab === 'ongoing' ? r.status !== 'completed' : r.status === 'completed')
    .filter(r => priorityFilter === 'all' ? true : normalizePriority(r.priority) === priorityFilter)
    .filter(r => departmentFilter === 'all' ? true : (r.department || 'Unassigned') === departmentFilter)
    .sort((a, b) => {
      const pMap = { High: 3, Moderate: 2, Low: 1 };
      if (sortBy === 'priority_high_to_low') {
        return (pMap[normalizePriority(b.priority)] || 0) - (pMap[normalizePriority(a.priority)] || 0);
      }
      if (sortBy === 'priority_low_to_high') {
        return (pMap[normalizePriority(a.priority)] || 0) - (pMap[normalizePriority(b.priority)] || 0);
      }
      if (sortBy === 'department_az') {
        return (a.department || 'Unassigned').localeCompare(b.department || 'Unassigned');
      }
      if (sortBy === 'department_za') {
        return (b.department || 'Unassigned').localeCompare(a.department || 'Unassigned');
      }
  ...
  const getStatusLabel = (status) => status === 'completed' ? 'closed' : status.replace('_', ' ');
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-white dark:bg-dark-800 p-1.5 rounded-2xl border border-slate-300 dark:border-white/10 shadow-2xl">
            ...
            >
              {t('active', 'Active')} ({reports.filter(r => r.status !== 'completed').length})
            </Button>
            <Button 
              ...
              className="rounded-xl px-6"
            >
              {t('closedTab', 'Closed')} ({reports.filter(r => r.status === 'completed').length})
            </Button>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between bg-slate-50 dark:bg-dark-800/50 p-4 rounded-2xl border border-slate-200 dark:border-white/5 backdrop-blur-sm">
             <h2 className="font-bold text-slate-900 dark:text-white uppercase tracking-widest text-xs flex items-center gap-2">
               ...
               {tab === 'ongoing' ? t('liveIncidentQueue', 'Live Incident Queue') : t('historicalArchive', 'Historical Archive')}
             </h2>
             <div className="flex items-center gap-2">
               ...
               >
                 <option value="newest">{t('latestFirst', 'Latest First')}</option>
                 ...
                 <option value="department_az">Department: A to Z</option>
                 <option value="department_za">Department: Z to A</option>
               </select>
               <select
                 className="bg-slate-50 dark:bg-dark-900 border border-slate-300 dark:border-white/10 text-xs rounded-xl px-4 py-2 outline-none focus:ring-2 ring-primary-500/30"
          ...
          {filteredReports.length === 0 ? (
            ...
            filteredReports.map((report) => (
              <Card key={report.id} className={cn("group transition-all duration-500 border-slate-200 dark:border-white/5 hover:border-primary-500/30", report.status === 'reopened' ? 'ring-2 ring-red-500/50 bg-red-500/5' : 'bg-white dark:bg-dark-800/40')}>
                ...
                {(expandedReportId === report.id || resolvingId === report.id) && (
                <CardContent className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-6 p-4 rounded-2xl bg-black/20 border border-slate-200 dark:border-white/5">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="p-3 bg-slate-100 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/5">
          ...
          <Card className="h-[650px] flex flex-col relative overflow-hidden bg-slate-50 dark:bg-dark-900 border-slate-300 dark:border-white/10 shadow-3xl sticky top-24">
            ...
            <div className="flex-1 w-full h-full relative">
              ...
              >
                ...
                {reports.filter(r => r.location).map((report) => (
                  <CircleMarker 
                    key={report.id} 
                    ...
                    <Tooltip direction="top" offset={[0, -10]} opacity={1} permanent={false}>
                      <div className="p-3 min-w-[200px] max-w-[250px] bg-slate-50 dark:bg-dark-900 border border-slate-300 dark:border-white/10 rounded-xl shadow-3xl space-y-2">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-white/5">
                          <span className="text-[9px] text-primary-400 font-bold uppercase">{getStatusLabel(report.status)}</span>
